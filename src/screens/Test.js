    ...
    const response = await fetch(`${Store.getInstance().IP}api/Debt`, {
      ...
      .then(response => {
        if (response['status'] == 200) {
          ...
          this.setState({
            checkMessage: 'Your bill for this month has been paid.',
            checkValid: false,
          });
        } else if (response['status'] == 401) {
          this.setState({
            checkMessage: 'Phone number not found.',
            checkValid: false,
          });
        }
      })
        ...
        <View style={{marginLeft: '8%'}}>
          ...
          <Text
            style={{
              ...
              fontSize: 21,
              marginBottom: 12,
            }}>{`Debt : $ ${this.state.phoneInfo['debt1']}`}</Text>
          <Text
            style={{
              color: Colors.white,
              fontSize: 17,
            }}>{`Payment Due Date : ${date.toLocaleDateString(
            'en-TR',
            options,
          )}`}</Text>
        ...
        <LinearGradient
          ...
          <View style={{flex: 1}}>
            ...
            <BottomNotification
              ...
              notificationType={formValid ? 'Successful' : 'Error'}
              firstLine={
                formValid
